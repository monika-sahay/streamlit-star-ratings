...
BASIC_EXAMPLE_FILE = ROOT_DIRECTORY / "star_ratings" / "example.py"
...
def test_should_render_template(page: Page):
    frame_0 = page.frame_locator(
        'iframe[title="star_ratings\\.star_ratings"]'
    ).nth(0)
...
def test_should_change_iframe_height(page: Page):
    frame = page.frame_locator('iframe[title="star_ratings\\.star_ratings"]').nth(1)

    expect(frame.get_by_text("Hello, Streamlit!")).to_be_visible()

    locator = page.locator('iframe[title="star_ratings\\.star_ratings"]').nth(1)
